...
static int ne_probe(struct ne *ne) {
	...
	printf("BOUND: %02x\n",inb(BOUNDARY)); 
   	outb(0x80,PAGESTOP);  //Stop Receive Buffer Ring
	...
	outb(0x46,CURRENT);//CURR Pointer in 0x26
...
int received_packet(){
	...
	printf("############ Inside received packet ############## \nNext Packet: 0x%02X CURR: 0x%02X\n",next_pkt,inb(COMMAND+NE_P1_CURR));
...
int readmem(void *dest,unsigned short src ,int n){
	...
	outb(REG_PAGE1, COMMAND);
	printf("\nInside Readmem, next_pkt=0x%02X, CURR:0x%02X \n",next_pkt,inb(COMMAND+NE_P1_CURR));
	...
	outb(REG_PAGE1, COMMAND);
	printf("\nCurrent pointer: 0x%02X \n",n=next_pack);
	outb(REG_PAGE0, COMMAND);
	outb(next_pack,BOUNDARY);
